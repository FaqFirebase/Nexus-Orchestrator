...
### v1.2.0
- **Dead code cleanup** — Removed unused imports, dead state, and no-op code across the server and frontend hooks with no behaviour changes. Removed unused `readConfig`, `writeConfig`, `getUserCount`, `fileURLToPath`, `__filename`/`__dirname`, and `hasAttachments` from `server.ts`. Dropped unused `ModelCategory` import from `useConfig.ts`. Removed unread `loadEnv`/`env` variable from `vite.config.ts`. Removed dead ping surface (`runPing`, `isPinging`, `pingResult`) from `useConnection.ts` — these had zero consumers. Removed `totalCount`, `activeConversationIdRef`, and `updateActiveConversationMessages` from `useConversations.ts`. Fixed a no-op ternary and removed the now-unused `isMasked`/`MASKED_PATTERN` helpers from `ProviderConfig.tsx`. Removed `getUserCount` export from `db.ts`. Removed `@types/express-rate-limit` dev dependency — `express-rate-limit@8` ships its own TypeScript types.
- **Code block horizontal scroll** — Code blocks in chat responses now scroll horizontally when content is wider than the container. Previously wide code was clipped with no way to reach the overflow. The Copy and language badge buttons are now positioned inside the block (`top-right`) so they remain reachable regardless of scroll position.
- **Provider URL canonicalization** — Changing a provider URL in the Models tab now automatically updates all category model assignments that referenced the old URL. Previously, `CategoryModel` stored the provider URL at assignment time, so renaming a provider left stale hostnames in category pools causing intermittent routing failures. The fix runs on every config save: any `providerUrl` that does not match a known provider is rewritten to the first local provider's URL.
- **Security: CVE patches** — Bumped `express-rate-limit` to `^8.5.1` to resolve `ip-address` XSS (GHSA-v2v4-37r5-5v8g). Added `overrides.postcss: ^8.5.10` to patch CSS stringify XSS in the transitive `postcss` dependency (GHSA-qx2v-qp2m-jg93). `npm audit` reports 0 vulnerabilities.
