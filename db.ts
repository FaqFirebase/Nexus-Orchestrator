...
function getCreateConversation() {
  return db.transaction((title: string, messages: any[]) => {
    const id = Date.now().toString();
    const updatedAt = new Date().toISOString();
    db.prepare('INSERT INTO conversations (id, title, updated_at) VALUES (?, ?, ?)').run(id, title || 'New Conversation', updatedAt);
    insertMessages(id, messages);
    return { id, title: title || 'New Conversation', messages, updatedAt };
  });
}
...
export function createConv(title: string, messages: any[]): any {
  return getCreateConversation()(title, messages);
}
